...
int main() {
    // Caches enabled here -- Disable if you want alternate measurements
	...
	for (i = 0; i < NUMBER_OF_TRIALS; i++) {
		...
		//XGpio_DiscreteWrite(&Gpio, LED_CHANNEL, 0x0); //Turns off one LED
		// Data = buffer[Addr];
		xil_printf("ABCDEFGHIF \n");
	...
	/*
	for (i = 0; i < NUMBER_OF_TRIALS; i++) {
		Addr = rand() % BUFFER_SIZE;

		// Measure for different repetition counts
		for (int reps = 5; reps <= 40; reps += 5) {
	    	timer_val_before = XTmrCtr_GetTimerCounterReg(XPAR_TMRCTR_0_BASEADDR, 1);
	    	...
	    	switch (reps) {
	        	case 5:
	            	...
	            	break;
	        	case 10:
	            	REPEAT_10(Data = buffer[Addr]);
	            	break;
	        	case 15:
	        		REPEAT_15(Data = buffer[Addr]);
	        		break;
	        	case 20:
	            	REPEAT_20(Data = buffer[Addr]);
	            	break;
	        	case 25:
	    	...
	    	int elapsed_time = XTmrCtr_GetTimerCounterReg(XPAR_TMRCTR_0_BASEADDR, 1) - timer_val_before;
	    	xil_printf("Reps: %d, Time: %d\n\r", reps, elapsed_time);
		}
	}
	*/
